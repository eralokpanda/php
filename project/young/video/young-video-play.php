...
$l=0;
$query=mysqli_query($db,"SELECT * FROM `young-video` WHERE sl_no='".$_GET['play']."'");
$sql=mysqli_fetch_array($query);
$q=mysqli_query($db,"SELECT * FROM `young-video-comment` WHERE video_code='".$sql['link']."' ORDER BY sl_no DESC LIMIT ".$l.",10");
ob_start();
$video_code=$sql['link'];
$views= $sql['views'];

$likenum=mysqli_num_rows(mysqli_query($db,"SELECT `like` FROM `like_unlike`"));	
...
<div style="float:right; padding:5px; margin-top:5px;" id="unlike-posted"><?php echo $sql['unlike']; ?></div>
...
<?php 
$query=mysqli_query($db,"SELECT sl_no,link,name,views FROM `young-video`  ORDER BY view_time DESC LIMIT 5 ");
...
if(isset($_POST['comment'])&&(isset($_SESSION['user']))&&($_POST['comment-text']!=''))
{
	...
	$inst="INSERT INTO `young-video-comment` (`fname`, `lname`, `user`, `comment`, `video_code`) VALUES ('".$_SESSION['fname']."', '".$_SESSION['lname']."', '".$_SESSION['user']."', '".$comment." ', '".$video_code."')";
...
$views += 1;
mysqli_query($db,"UPDATE `young-video` SET view_time = '".date("Y/m/d H:i:s",time())."', views = '".$views."' WHERE sl_no='".$_GET['play']."'");
?>
