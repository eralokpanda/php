<?php
$query=mysqli_query($db,"SELECT * FROM `young-game` WHERE sl_no='".$_GET['play']."'");
...
$l=0;
$q=mysqli_query($db,"SELECT * FROM `young-game-comment` WHERE game_code='".$sql['link']."' ORDER BY sl_no DESC LIMIT ".$l.",10");
ob_start();
$views= $sql['views'];
$likeno=mysqli_num_rows(mysqli_query($db,"SELECT `like` FROM `like_unlike` WHERE page = 'young-game'"));
?>
...
<?php 
$query=mysqli_query($db,"SELECT sl_no,link,name,views FROM `young-game`  ORDER BY view_time DESC LIMIT 5 ");
while($sql=mysqli_fetch_array($query))
{
?>
<a href="./index.php?page=young-game-play&play=<?php echo $sql['sl_no']; ?>">
<div class="list-game">
...
var x='like='+1;
		...
		data:x,
		cache:false,
		url:"./young/game/like-unlike-ajax.php",
		success: function(e){
			if(e)
			$("#like-posted").html(e)
...
if(isset($_POST['comment'])&&(isset($_SESSION['user']))&&($_POST['comment-text']!=''))
{
	...
	$inst="INSERT INTO `young-game-comment` (`fname`, `lname`, `user`, `comment`, `game_code`) VALUES ('".$_SESSION['fname']."', '".$_SESSION['lname']."', '".$_SESSION['user']."', '".$comment." ', '".$game_code."')";
...
$views += 1;
mysqli_query($db,"UPDATE `young-game` SET view_time = '".date("Y/m/d H:i:s",time())."', views = '".$views."' WHERE sl_no='".$_GET['play']."'");
?>