...
$l=0;
$query=mysqli_query($db,"SELECT * FROM `senior-video` WHERE sl_no='".$_GET['play']."'");
$sql=mysqli_fetch_array($query);
$q=mysqli_query($db,"SELECT * FROM `senior-video-comment` WHERE video_code='".$sql['link']."' ORDER BY sl_no DESC LIMIT ".$l.",10");
$vdo_code=$sql['link'];
ob_start();

$views= $sql['views'];

$likeno=mysqli_num_rows(mysqli_query($db,"SELECT `like` FROM `like_unlike`"));	
...
<div class="prog-bar">
<div class="count"><?php echo $views; ?>
</div>
...
<?php 
$query=mysqli_query($db,"SELECT sl_no,link,name,views FROM `senior-video`  ORDER BY view_time DESC LIMIT 5 ");
while($sql=mysqli_fetch_array($query))
{
?>
<a href="./index.php?page=senior-video-play&play=<?php echo $sql['sl_no']; ?>">
...
var x='like='+1;
		...
		data:x,
		cache:false,
		url:"./young/like-unlike-ajax.php",
		success: function(e){
			if(e)
			$("#like-posted").html(e)
...
if(isset($_POST['comment'])&&(isset($_SESSION['user']))&&($_POST['comment-text']!=''))
{
	...
	$inst="INSERT INTO `senior-video-comment` (`fname`, `lname`, `user`, `comment`, `video_code`) VALUES ('".$_SESSION['fname']."', '".$_SESSION['lname']."', '".$_SESSION['user']."', '".$comment." ', '".$vdo_code."')";
...
$views += 1;
mysqli_query($db,"UPDATE `senior-video` SET view_time = '".date("Y/m/d H:i:s",time())."', views = '".$views."' WHERE sl_no='".$_GET['play']."'");
?>
